...
//去除验证函数 ValidatorOnChange();
if(ev.indexOf("ValidatorOnChange();")> 0){
    ev = ev.substring(ev.indexOf("ValidatorOnChange();") + 20 , ev.lastIndexOf("}"));
}
else if(ev.indexOf("ValidatorOnChange(event);")> 0){ //.net v2.0 的验证事件
    ev = ev.substring(ev.indexOf("ValidatorOnChange(event);") + 25 , ev.lastIndexOf("}"));
}
else{
    ev = ev.substring(ev.indexOf("{") + 1 , ev.lastIndexOf("}"));
}
//    ev = ev.substring(
//			((ev.indexOf("ValidatorOnChange();")> 0) ? ev.indexOf("ValidatorOnChange();") + 20 
//			: ev.indexOf("{") + 1), ev.lastIndexOf("}"));//去除验证函数 ValidatorOnChange();
...
Calendar.prototype.show = function (dateObj, popControl) {
  ...
  if(this.panel.innerHTML==""||cal.dateFormatStyleOld != cal.dateFormatStyle)//2008-01-29 把构造表格放在此处
  {
	this.draw();
	this.bindYear();
	this.bindMonth();
